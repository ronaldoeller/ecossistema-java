...
**Contexto:** Você já trabalha com gestão de projetos e requisitos em uma stack Java Spring Boot + Angular + Oracle + Redis, com experiência prévia em sistemas de mensageria (originalmente IBM MQ). Isso é uma vantagem enorme — você já conhece os fluxos de negócio, a arquitetura geral e o vocabulário técnico. O plano abaixo foca em transformar esse conhecimento passivo em habilidade prática de codificação.

**Duração estimada:** 9 a 12 meses, estudando de 8 a 12h por semana (ajuste conforme sua disponibilidade).
...
- Versão **Community** (gratuita) já cobre praticamente tudo que você precisa nas Fases 0 a 6; a versão **Ultimate** (paga, mas com licença gratuita para estudantes e trials) adiciona suporte nativo a Spring Boot (visualização de beans, endpoints REST navegáveis), banco de dados integrado e suporte a JavaScript/Angular — vale considerar quando chegar nas fases mais avançadas
- Plugins essenciais: Lombok (reduz boilerplate em Java, muito usado em projetos Spring), SonarLint (feedback do Sonar direto na IDE, conecta com a Fase 6), .env/dotenv se for usar variáveis de ambiente
- Recursos a explorar desde cedo: debugger visual, refatorações automáticas (extrair método, renomear com segurança — conecta direto com a Fase 2 de Clean Code), atalhos de navegação (`Ctrl+Click`, `Ctrl+Shift+F`), Git integrado (commits, branches e resolução de conflitos direto na IDE)
...
- Extensões essenciais: **Angular Language Service** (autocomplete e checagem de tipos em templates HTML do Angular), **ESLint** (conecta direto com a Fase 6 de qualidade de código), **Prettier** (formatação automática, padrão de mercado para times frontend), **GitLens** (histórico e blame do Git direto no editor)
...
## Fase 2 — Clean Code e Boas Práticas (2 a 3 semanas)
...
## Fase 3 — Oracle Database e JPA avançado (3 a 4 semanas)
...
## Fase 4 — Testes automatizados: JUnit e Mockito (2 a 3 semanas)
...
## Fase 5 — TDD (Test-Driven Development) (2 a 3 semanas)
...
- Como o TDD influencia o design: classes mais coesas, menor acoplamento, interfaces mais claras (conecta diretamente com o que você viu em Clean Code/SOLID na Fase 2)
...
- Comparar a experiência com o jeito "código primeiro, teste depois" que você já praticou na Fase 4 — vale documentar essa reflexão, é um ótimo ponto para falar em entrevista
...
## Fase 6 — Qualidade de Código e CI/CD (2 a 3 semanas)

Até aqui você já escreve código limpo (Fase 2) e testado (Fases 4 e 5). Agora é hora de **automatizar a verificação dessa qualidade** e o processo de entrega — prática padrão em qualquer empresa de tecnologia madura, e praticamente obrigatória em projetos governamentais por causa de exigências de auditoria e rastreabilidade.
...
## Fase 8 — RabbitMQ e mensageria (2 a 3 semanas)

**Tópicos:**
- Conceitos de mensageria assíncrona: filas, exchanges, bindings, routing keys, produtores/consumidores
- Tipos de exchange no RabbitMQ: direct, topic, fanout, headers — e quando usar cada um
- Spring AMQP com RabbitMQ: `@RabbitListener`, `RabbitTemplate`, configuração de filas e exchanges via `@Bean` ou `application.yml`
- Padrões comuns: dead-letter queue/exchange, retry com backoff, idempotência de consumidores, ack manual vs automático
- Serialização de mensagens (JSON via Jackson é o padrão de mercado) e versionamento de contratos de mensagem
- Quando usar mensageria vs chamada síncrona (decisão arquitetural que você já vê na prática como analista)

**Prática recomendada:**
- Subir o RabbitMQ localmente via Docker (`docker run -p 5672:5672 -p 15672:15672 rabbitmq:management` — a imagem `management` já vem com painel web de administração)
- Simular um fluxo assíncrono: um serviço publica uma mensagem (ex: "solicitação criada") e outro serviço consome e processa
- Explorar o painel de administração do RabbitMQ (`localhost:15672`) para visualizar filas, mensagens e conexões em tempo real — ótimo para entender o que está acontecendo "por dentro"

**Recursos comuns no mercado:**
- Documentação oficial: [rabbitmq.com/tutorials](https://www.rabbitmq.com/tutorials) — os tutoriais oficiais (em várias linguagens) são a referência mais didática para entender os conceitos
- Documentação: [spring.io/projects/spring-amqp](https://spring.io/projects/spring-amqp)

**Observação:** RabbitMQ é hoje uma das ferramentas de mensageria mais usadas no mercado geral (ao lado de Kafka, mais voltado a streaming de eventos em alto volume), então esse conhecimento é bem mais transferível entre empresas do que era o IBM MQ, que tende a aparecer mais em ambientes corporativos/legados específicos.

---

## Fase 9 — Angular (Frontend) (6 a 8 semanas, pode rodar em paralelo com Fase 3-8)
...
## Fase 10 — Docker e Docker Compose (2 a 3 semanas)

Até aqui você já usou Docker de forma pontual (para subir Oracle, Redis, RabbitMQ localmente). Agora é hora de entender a ferramenta a fundo — é praticamente unânime no mercado que "saber Docker" é requisito básico para qualquer vaga backend hoje em dia.

**Tópicos:**
- Conceitos fundamentais: imagem vs container, camadas (layers), volumes (persistência de dados), redes (networks)
- Dockerfile: escrever o seu próprio, para empacotar a aplicação Spring Boot (multi-stage build é a prática de mercado padrão, para gerar imagens menores e mais seguras)
- Docker Compose: orquestrar múltiplos containers com um único `docker-compose.yml` (aplicação + Oracle + Redis + RabbitMQ, tudo subindo junto com um comando)
- Variáveis de ambiente e arquivos `.env` para configuração por ambiente (dev/hml/prod)
- Volumes nomeados vs bind mounts — quando usar cada um (ex: persistir dados do Oracle entre reinícios do container)
- Comandos essenciais do dia a dia: `docker compose up -d`, `docker compose logs -f`, `docker compose down -v`, `docker exec` para entrar em um container rodando
- Boas práticas de imagem: `.dockerignore`, usuário non-root, imagens base enxutas (ex: `eclipse-temurin` com Alpine para Java)

**Prática recomendada:**
- Escrever um Dockerfile para a API Spring Boot construída nas fases anteriores
- Criar um `docker-compose.yml` que sobe a aplicação junto com Oracle, Redis e RabbitMQ — um único comando (`docker compose up`) deixa todo o ambiente de desenvolvimento pronto
- Repetir o mesmo processo para o frontend Angular (build de produção servido via Nginx é o padrão de mercado)

**Recursos comuns no mercado:**
- Documentação oficial: [docs.docker.com](https://docs.docker.com)
- Curso: "Docker do Zero" (várias opções em pt-BR na Alura/Udemy costumam ser suficientes para o nível necessário aqui)

**Critério de avanço:** conseguir, a partir de um projeto do zero, escrever o Dockerfile e o docker-compose.yml necessários para rodar toda a stack localmente com um único comando.

---

## Fase 11 — Arquitetura de Microsserviços e Kubernetes (4 a 5 semanas)
...
- Comunicação entre serviços: síncrona (REST, gRPC) vs assíncrona (aqui entra o RabbitMQ da Fase 8, agora como peça de integração entre serviços)
...
- Por que Kubernetes existe: o problema que ele resolve além do Docker Compose (orquestração em múltiplas máquinas, self-healing, escalonamento automático, rollout/rollback controlado)
- Conceitos fundamentais: Pod, Deployment, Service, Namespace, ConfigMap e Secret (para configuração e dados sensíveis)
- Diferença prática entre Docker Compose (bom para ambiente local/dev) e Kubernetes (voltado a produção e ambientes distribuídos) — entender quando cada um é apropriado é tão importante quanto saber configurar
- `kubectl`: comandos essenciais do dia a dia (`get`, `describe`, `logs`, `apply`, `exec`)
- Escrever manifests YAML básicos: Deployment + Service para expor um dos microsserviços construídos na Fase 11
- Ambiente local para praticar sem custo: Minikube ou Kind (Kubernetes in Docker)
- Noção de Helm (gerenciador de pacotes para Kubernetes) — não precisa aprofundar, mas é comum aparecer em vagas mais seniores
...
- Subir os serviços com Docker Compose (usando o conhecimento da Fase 10), incluindo Oracle, Redis e RabbitMQ como containers
- Adicionar um API Gateway simples na frente dos dois serviços
- Depois de validar tudo local com Docker Compose, subir o mesmo cenário no Minikube: escrever os manifests de Deployment e Service para cada microsserviço e observar o Kubernetes reiniciar um Pod automaticamente se ele "morrer" (self-healing)
...
- Documentação oficial: [kubernetes.io/docs](https://kubernetes.io/docs) — o tutorial interativo "Kubernetes Basics" no próprio site é um bom ponto de partida

**Observação importante:** microsserviços resolvem problemas de escala e times grandes, mas adicionam complexidade operacional real. O mesmo vale para Kubernetes: muitas empresas de médio porte rodam perfeitamente bem com Docker Compose ou soluções mais simples, e só migram para Kubernetes quando a escala realmente justifica. Em entrevistas, saber explicar **os trade-offs** (não só a implementação) costuma impressionar mais do que saber decorar comandos de `kubectl`.

**Critério de avanço:** conseguir explicar, com argumentos técnicos, quando você recomendaria microsserviços versus monólito, e Kubernetes versus Docker Compose — e ter ao menos dois serviços simples rodando tanto via Docker Compose quanto via manifests básicos de Kubernetes (Minikube).

---

## Fase 12 — Projeto integrador final (4 a 6 semanas)
...
- Serviço de "solicitações": Spring Boot com Oracle + Flyway, código estruturado seguindo os princípios de Clean Code/SOLID da Fase 2
- Serviço de "notificações": consome mensagens de fila (RabbitMQ) e processa de forma assíncrona
...
- Ambiente local com Docker Compose subindo tudo junto (serviços + Oracle + Redis + RabbitMQ)
- Manifests básicos de Kubernetes (Deployment + Service) para pelo menos um dos microsserviços, rodando em Minikube — para demonstrar noção de orquestração além do ambiente local
...
| 2 | Clean Code e Boas Práticas | 2-3 semanas |
| 3 | Oracle + JPA | 3-4 semanas |
| 4 | JUnit + Mockito | 2-3 semanas |
| 5 | TDD | 2-3 semanas |
| 6 | Sonar, Lint e CI/CD | 2-3 semanas |
| 7 | Redis | 2 semanas |
| 8 | RabbitMQ | 2-3 semanas |
| 9 | Angular | 6-8 semanas (paralelo) |
| 10 | Docker e Docker Compose | 2-3 semanas |
| 11 | Arquitetura de Microsserviços e Kubernetes | 4-5 semanas |
| 12 | Projeto integrador | 4-6 semanas |

**Total:** aproximadamente 9 a 12 meses, dependendo do ritmo e da possibilidade de paralelizar o frontend com o backend.
